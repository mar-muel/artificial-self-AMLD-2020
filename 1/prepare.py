...
def process_zip(name, regs, args):
    with open(os.path.join(args.output_dir, name + '.txt'), 'w+') as fh:
...
if args.type in ['all','tweets']: # parse trump tweets
    ...
    df = df[filter]
    df.sample(args.num_samples).text.to_csv(os.path.join(args.output_dir, 'tweets.txt'), index=False, header=False, quoting=csv.QUOTE_NONE, escapechar="\\", sep="\\")
...
if args.type in ['all','chess']: # parse chess games
    ...
    with open(os.path.join(args.output_dir, 'chess.txt'),'w+') as fh:
...
if args.type in ['all','music']: # parse abc songs
    ...
    with open(os.path.join(args.output_dir, 'music.txt'),'w+') as fh:
...
if args.type in ['all','shakespeare']: # parse shakespeare plays
    ...
    df.sample(args.num_samples).PlayerLine.to_csv(os.path.join(args.output_dir, 'shakespeare.txt'), index=False, header=False, quoting=csv.QUOTE_NONE, escapechar="\\", sep="\\")
...
if args.type in ['all','html']: # parse html
    ...
    with open(os.path.join(args.output_dir, 'html.txt'),'w+') as fh:
